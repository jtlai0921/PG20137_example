                           ...
                           float MaxSize, long MaxPolygons)
{
  ...
  for(i=0;i<m_NumGroups;i++) {
    if(m_Groups[i].NumPolygons != 0)
      Graphics->GetDeviceCOM()->CreateIndexBuffer(
...
BOOL cNodeTreeMesh::Render(cFrustum *Frustum, float ZDistance)
{
  ...
  m_Graphics->GetDeviceCOM()->SetStreamSource(0, pVB, m_VertexSize);
  m_Graphics->GetDeviceCOM()->SetVertexShader(m_VertexFVF);
  ...
  for(i=0;i<m_NumGroups;i++) {
    ...
    if(m_Groups[i].NumPolygonsToDraw) {
      ...
      m_Graphics->GetDeviceCOM()->SetIndices(m_Groups[i].IndexBuffer, 0);
