  ...
  for(Layer=0;Layer<m_NumLayers;Layer++) {
    ...
    for(Row=0;Row<NumRows+1;Row++) {
      for(Column=0;Column<NumColumns+1;Column++) {
        TileNum = MapPtr[(Column + YPos)*m_Width+Row+XPos];
        TileX = Row    * m_Tiles->GetWidth(0) - XOff;
        TileY = Column * m_Tiles->GetHeight(0) - YOff;

        m_Tiles->Draw(0, TileNum, TileX*2.5, TileY*2.5, 0xFFFFFFFF, 2.5f, 2.5f);
      }
    }
